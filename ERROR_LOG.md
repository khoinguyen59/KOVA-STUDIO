...
* **Cách xử lý (Fix Details):**
  * Trên POSIX/Colab, `tts_processor` dùng `ffmpeg` hệ thống (`shutil.which`) thay vì file `.exe`; notebook cài và kiểm tra FFmpeg trước khi khởi động server, đồng thời `git pull --ff-only` khi runtime còn clone cũ để server được cập nhật code fix.
